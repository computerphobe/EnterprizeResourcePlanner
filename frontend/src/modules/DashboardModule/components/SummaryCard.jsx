import { Tag, Divider, Row, Col, Spin, Tooltip, Progress } from 'antd';
import { useMoney } from '@/settings';
import { selectMoneyFormat } from '@/redux/settings/selectors';
import { useSelector } from 'react-redux';
import './summary_card.css';

export default function AnalyticSummaryCard({ title, tagColor, data, prefix, isLoading = false, percentageChange }) {
                  ...
                  {!isNaN(percentageChange) && (
                    <div className="summary-card-progress">
                      <Progress
                        ...
                        format={(percent) =>
                          `${percentageChange >= 0 ? '↑' : '↓'} ${percent?.toFixed(0)}%`
                        }
                        width={70}
                        strokeColor={percentageChange >= 0 ? '#52c41a' : '#ff4d4f'}
                        trailColor="#f0f0f0"
                      />
                    </div>
                  )}
